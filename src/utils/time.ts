 ...
 * @returns 该日期对应的时间戳 单位毫秒
 */
export const getTimeStamp = (date?: string) => {
  // ios 不支持 '2014-07-10 10:21:12' 这样的格式
  const dateString: string = new Date(date ? date.replace(/-/g, '/') : new Date()).toString();
...
type ForMatterDateType = 'YMD' | 'YMDHMS';

export const forMatterDate = (
  timeStamp: any,
  options?: {
    type: ForMatterDateType;
  },
) => {
  const { type } = options || { type: 'YMDHMS' };
  ...
  if (type === 'YMDHMS') return `${year}-${month}-${date} ${hour}:${minute}:${second}`;
  if (type === 'YMD') return `${year}-${month}-${date}`;
};

export const formatTime = (time: string) => {
  return forMatterDate(new Date(Number(`${time}000`)));
};
 ...
 * @return {*}
 */
export function formatTimeScoped(arr: string[], format = 'YYYY-MM-DD') {
  const map = {
    0: '今日',
    1: '昨日',
    2: '前日',
  };
  const start = dayjs(arr[0], format);
  const end = dayjs(arr[1], format);
  const startDiff = dayjs().diff(start, 'days');
  const endDiff = dayjs().diff(end, 'days');
  return `${start.format('YYYY-MM-DD')}~${end.format('YYYY-MM-DD')} | ${map[startDiff] || `过去${startDiff}天`}~${
    map[endDiff] || `过去${endDiff}天`
  }`;
}
