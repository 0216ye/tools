      ...
      >
        ...
        <div :draggable="!item.noDrag">
          ...
          <transition name="fade-drag">
            <div
              v-if="item.dragId === draggedId"
              class="list-item-active"
            />
          </transition>
        </div>
        <template v-if="!item.noDrag && draggedId !== null && draggedId !== item.dragId">
          <!-- 放置显示器，前 -->
          <div
  ...
  methods: {
    ...
    handleDrop(event) {
      ...
      if (event.target.classList.contains('drag-zone')) {
        ...
        this.dragItems.splice(index, 0, this.dragItems.splice(this.draggedIndex, 1)[0]);
